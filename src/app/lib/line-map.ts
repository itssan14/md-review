export interface LineMapEntry {
  startLine: number;
  endLine: number;
  type: "heading" | "paragraph" | "code" | "mermaid" | "list" | "blockquote" | "table" | "hr";
  text: string;
  _used?: boolean;
}
...
export function matchBlockToMap(el: Element, lineMap: LineMapEntry[]): LineMapEntry | null {
  ...
  if (tag === "p") {
    const entry = lineMap.find(
      (m) => m.type === "paragraph" && !m._used && first40 && m.text.slice(0, 30).trim() !== "" && first40.startsWith(m.text.slice(0, 30).trim()),
    );
    if (entry) { entry._used = true; return entry; }
